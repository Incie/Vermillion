...
#define ATTACK(r, d, a) new ActionAttack(level, actor, r, d, a)
#define MOVE(m) new ActionMove(level, actor, m)
#define SHIELD(s) new ActionShieldSelf(level, actor, s)
	...
	: level(level)
{
	playerCards = std::vector<PlayerCard>{
		PlayerCard{
			...
			{CardAbility{"Move", 4}}, 
			17,	6,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{new ActionAttack(level, actor, 1, 4, 1), new ActionShieldSelf(level, actor, 1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(4)};  return new PlayerRound(v); }
		},
		PlayerCard{
			...
			29,	5,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(1, 2, 1), MOVE(2), ATTACK(1,2,1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(3, 3, 1)};  return new PlayerRound(v); }
		},
		PlayerCard{
			"Unstoppable Charge",
			...
			86, 4,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(1,5,1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(4)};  return new PlayerRound(v); }
		},
		PlayerCard{
			"Juggernaut",
			...
			{CardAbility{"Move", 4}},
			34, 2,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(2), ATTACK(1,2,1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(4)};  return new PlayerRound(v); }
		},
		PlayerCard{
			"Hook and Chain",
			...
			{CardAbility{"Move", 4}},
			42, 3,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(2,3,1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(4)};  return new PlayerRound(v); }
		},
		...
		PlayerCard{
			...
			54, 1,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(1,3,2)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(3)};  return new PlayerRound(v); }
		},
		PlayerCard{
			"Trample",
			...
			72, 1,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(1,3,1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(4), ATTACK(1,2,1)};  return new PlayerRound(v); }
		},
		PlayerCard{
			"Skewer",
			{CardAbility{"Attack", 3}, CardAbility{"Range", 3, true}, CardAbility{"PULL", 2, true}},
		...
		PlayerCard{
			...
			27, 1,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(3,3,1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(1,2,1)};  return new PlayerRound(v); }
		},
		PlayerCard{
			"Provoking Roar",
			...
			10, 1,
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{ATTACK(1,2,1)};  return new PlayerRound(v); },
			[](Level& level, Actor& actor) {auto v = std::vector<Action*>{MOVE(2)};  return new PlayerRound(v); }
		},
	};
}
